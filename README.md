...
The **Account Service** is a core component of the **Video Streaming Platform**, responsible for managing user accounts and ensuring a consistent, secure experience across the platform. Key responsibilities include:

- **User Registration**: Handles the onboarding of new users, ensuring secure and seamless account creation.
- **Account Management**: Supports user account updates, including email and personal information.
- **Account Deletion**: Facilitates secure account deactivation and deletion processes, respecting user privacy and compliance requirements.
...
4. Start the service:
   ...
   The service will start on http://localhost:8110 using the embedded Tomcat web server.
...
1. Build the Docker image:
    ```bash
   docker build -t streamix-users-account:latest .
    ```
2. Run the container:
    ```bash
   docker run --rm --name streamix_account -p 8110:8110 streamix-users-account:latest
    ```
